...
class NgramModel(ModelI):
    def __init__(self, hparams):
        ...
        if model_class_name in ["Lidstone", "Laplace"]:
            ...
            self.model = model_class(self.order, self.vocab, self.counter, **kwargs)
    ...
    def log_score(self, progression):
        progression = list(pad_sequence(progression, self.order, pad_left=True, left_pad_symbol="<s>", pad_right=True, right_pad_symbol="</s>"))
    ...
    def generate(self, min_length=1, max_length=100):
        ...
        while not finished:
            ...
            finished = next_chord == "</s>" and len(progression) > min_length or len(progression) > max_length
